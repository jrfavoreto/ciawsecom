...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($nome_err) && empty($login_err) && empty($senha_err) && empty($niveis_acesso_err)){
		...
		$stmt2 = mysqli_prepare($conn_cfo, $sql);
		mysqli_stmt_bind_param($stmt2, "sssi", $param_nome, $param_login, $param_senha, $param_niveis_acesso);
		mysqli_stmt_execute($stmt2);

	    	$stmt3 = mysqli_prepare($conn_rm2, $sql);
		mysqli_stmt_bind_param($stmt3, "sssi", $param_nome, $param_login, $param_senha, $param_niveis_acesso);
		mysqli_stmt_execute($stmt3);

	    	$stmt4 = mysqli_prepare($conn_rm3, $sql);
		mysqli_stmt_bind_param($stmt4, "sssi", $param_nome, $param_login, $param_senha, $param_niveis_acesso);
